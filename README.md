...
![](https://github.com/kireetijosyula41/Object_Recognizer_and_Retriever/blob/main/HandTrackingHQ.gif)
...
AS A SIDE NOTE: There are two versions of the computer version model, which is the pretrained version, and a custom version which we attempted to train. The code in obj_dect defaults to the pretrained version, as the performance is significantly better than compared to the custom version. However, the results in the two images above are from the custom version, showing moderate success in the object detection frame. 
